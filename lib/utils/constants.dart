abstract class Constants {
  static const int backgroundColorRed = 255;
  static const int backgroundColorGreen = 0;
  static const int backgroundColorBlue = 217;
  static const int backgroundColorOpacity = 255;
  static const int bodyCrossAxisCount = 2;
  static const double bodyPadding = 25.0;
  static const double bodyMainAxisSpacing = 15.0;
  static const double bodyCrossAxisSpacing = 40.0;
  static const int bodyChildren = 6;
  static const double bodyBorderRadius = 25.0;
  static const double bottomNavigationBarChildren = 25.0;

  static const double textStyleFontSize = 28.0;
  static const double textStyleHeight = 2.0;
  static const double textStyleFontSizeSmall = 25.0;
  static const double textStyleWithColorFontSize = 25.0;
  static const int textStyleRed = 189;
  static const int textStyleGreen = 56;
  static const int textStyleBlue = 27;
  static const double textStyleOpacity = 2.0;

  static const List<int> bodyDiceInitial = [1, 2, 3, 4, 5, 6];
  static const String bodyScoreInitial = "21";
  static const int diceRollValue = 6;
}
